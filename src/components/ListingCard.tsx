      ...
      <Link to={`/listings/${listing.id}`} className="group block">
        <div className="card-hover rounded-2xl overflow-hidden bg-card border border-border">
          ...
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            <img
              src={imageError ? "/placeholder.svg" : (listing.image as any)?.src || listing.image}
              alt={listing.title}
              className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${imageLoading ? 'hidden' : ''}`}
