...
export default function ListingDetailPage() {
  ...
  const { data: listing, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (listingData && supabase) {
          // Only attempt Supabase fetch if the ID looks like a UUID
          const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id as string);
  ...
  const { data: eligibleBookings = [] } = useReviewEligibility(id as string);

  const canViewContact = listing?.canViewContact || false;
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    const res = await createReview({
      listing_id: id,
      booking_id: eligibleBookings[0]?.id || "",
  ...
  return (
    <div className="page-wrapper py-6 md:py-10">
      ...
      {/* Gallery */}
      <div className="mb-10">
        <GallerySlider images={listing.gallery && listing.gallery.length ? listing.gallery : (listing.image ? [listing.image] : [])} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <section className="space-y-6">
            <HostContact
              hostContact={listing.hostContact}
          ...
          {canViewContact && (
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-display font-semibold">Chat with Host</h2>
                <button
              ...
              {showChat && (
                <ChatPanel
                  ...
                  receiverName={listing.owner.name}
                />
              )}
            </section>
          )}
