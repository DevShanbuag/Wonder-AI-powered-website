...
import { CATEGORIES, SEASONS, AMENITIES, Amenity } from "@/lib/types";
...
export default function AddResortPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const coordsParts = formData.coordinates.split(",").map(c => parseFloat(c.trim()));
      if (coordsParts.length !== 2 || coordsParts.some(isNaN)) {
        throw new Error("Invalid coordinates format. Please use 'Longitude, Latitude' (e.g., 77.1892, 32.2396)");
      }
      
      // Ensure it's stored as [lng, lat] to match our api.ts auto-swapping logic
      // India Longitude is always > Latitude.
      const coords = coordsParts[0] > coordsParts[1] 
        ? [coordsParts[0], coordsParts[1]] as [number, number]
        : [coordsParts[1], coordsParts[0]] as [number, number];
      ...
      toast.error(err.message || "Failed to register resort");
    } finally {
      setLoading(false);
    }
  };
