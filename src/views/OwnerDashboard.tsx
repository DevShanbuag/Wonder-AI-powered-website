  ...
  useEffect(() => {
    ...
    const loadOwnerData = async (userId: string) => {
      ...
      if (error) {
        ...
        const mappedListings = data.map((d: Resort) => ({
          id: d.id,
          title: d.title,
          price: d.price,
        ...
        const totalRevenue = mappedListings.reduce((acc: number, curr: Listing) => acc + (curr.price * 5), 0); // Placeholder: 5 bookings each
        const totalBookings = mappedListings.length * 5;
        const avgRating = mappedListings.length > 0 
          ? mappedListings.reduce((acc: number, curr: Listing) => acc + curr.avgRating, 0) / mappedListings.length
          : 0;
