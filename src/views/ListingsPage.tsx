...
import AiResortChat from "@/components/AiResortChat";
...
export default function ListingsPage() {
  ...
  const [userLocation, setUserLocation] = useState<{ lat: number; lon: number } | undefined>(
    initialLat && initialLon ? { lat: initialLat, lon: initialLon } : undefined
  );

  const guestsParam = searchParams.get("guests") ? parseInt(searchParams.get("guests")!) : 1;
  ...
  useEffect(() => {
    ...
    if (locName) setSearch(locName);
    if (lat && lon) setUserLocation({ lat: parseFloat(lat), lon: parseFloat(lon) });
  }, [searchParams]);
  ...
  const filtered = useMemo(() => {
    ...
    if (userLocation) {
      const d = (l: typeof results[number]) =>
        Math.sqrt(
          Math.pow(l.coordinates[0] - userLocation.lat, 2) +
          Math.pow(l.coordinates[1] - userLocation.lon, 2)
        );
    ...
    results = results.filter((l) => l.capacity.guests >= guestsParam);
  ...
  return (
    <div>
      ...
      <section className="relative h-[420px] md:h-[500px] overflow-hidden">
        <img src={(heroBg as any).src || heroBg} alt="Wonder Stay hero" className="absolute inset-0 w-full h-full object-cover" />
